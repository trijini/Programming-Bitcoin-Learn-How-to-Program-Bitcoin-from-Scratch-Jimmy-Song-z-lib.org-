...
class NetworkEnvelope:
    ...
    def parse(cls, s, testnet=False):
        ...
        payload_len = int.from_bytes(s.read(4), 'little')
        # checksum 4 bytes, first four of hash256 of payload
        checksum = s.read(4)
        # payload is of length payload_length
        payload = s.read(payload_len)
        # verify checksum
        sanity = hash256(payload)[:4]
        print()
        if sanity != checksum:
            print(f'Cheksum not matching')
...
class VersionMessage:
    ...
    def serialize(self):
        ...
        s += self.services.to_bytes(8, 'little')
        # timestamp is 8 bytes little endian
        s += self.timestamp.to_bytes(8, 'little')
        # receiver services is 8 bytes little endian
        s += self.receiver_services.to_bytes(8, 'little')
        ...
        s += self.sender_services.to_bytes(8, 'little')
        ...
        s += self.latest_block.to_bytes(4, 'little')
